...
namespace zoltan_csizmadia {
	...
	// From wide char string to wstring
	void SystemInfoUtils::LPCWSTR2wstring(LPCWSTR strW, wstring& str)
	{
#ifdef UNICODE
	...
	void SystemInfoUtils::Unicode2wstring(UNICODE_STRING* strU, wstring& str)
	{
		if (*(DWORD*)strU != 0)
			LPCWSTR2wstring((LPCWSTR)strU->Buffer, str);
		else
			str = _T("");
	}

	// From device file name to DOS filename
	BOOL SystemInfoUtils::GetFsFileName(LPCTSTR lpDeviceFileName, wstring& fsFileName)
	{
		BOOL rc = FALSE;
	...
	BOOL SystemInfoUtils::GetDeviceFileName(LPCTSTR lpFsFileName, wstring& deviceFileName)
	{
		BOOL rc = FALSE;
		TCHAR lpDrive[3];
	...
	{
		m_ProcessInfos.clear();// RemoveAll();
		...
		do
		{
			...
			m_ProcessInfos[pSysProcess->dUniqueProcessId] = pSysProcess;// SetAt(pSysProcess->dUniqueProcessId, pSysProcess);
	...
	BOOL SystemThreadInformation::Refresh()
	{
		...
		for each(SystemHandleInformation::SYSTEM_HANDLE h in hi.m_HandleInfos)
		{
			...
			if (SystemHandleInformation::GetThreadId(ti.ThreadHandle, ti.ThreadId, ti.ProcessId))
				m_ThreadInfos.push_back(ti); // AddTail(ti);
		}
	...
	const wstring& SystemHandleInformation::GetFilter()
	{
		...
		for (i = 0; i < pSysHandleInformation->Count; i++)
		{
			...
			if (pSysHandleInformation->Handles[i].ProcessID == m_processId || m_processId == (DWORD)-1)
			{
				...
				if (bAdd)
				{
					...
					m_HandleInfos.push_back(pSysHandleInformation->Handles[i]);// AddTail(pSysHandleInformation->Handles[i]);
	...
	BOOL SystemHandleInformation::GetTypeToken(HANDLE h, wstring& str, DWORD processId)
	{
		ULONG size = 0x2000;
		UCHAR* lpBuffer = NULL;
		BOOL ret = FALSE;
	...
	BOOL SystemHandleInformation::GetName(HANDLE handle, wstring& str, DWORD processId)
	{
		WORD type = 0;
	...
	BOOL SystemHandleInformation::GetNameByType(HANDLE h, WORD type, wstring& str, DWORD processId)
	{
		ULONG size = 0x2000;
		UCHAR* lpBuffer = NULL;
		BOOL ret = FALSE;
		...
		{
			SystemInfoUtils::Unicode2wstring((UNICODE_STRING*)lpBuffer, str);
			ret = TRUE;
		}
	...
	BOOL SystemHandleInformation::GetProcessPath(HANDLE h, wstring& strPath, DWORD remoteProcessId)
	{
	...
	void SystemModuleInformation::GetModuleListForProcess(DWORD processID)
	{
		...
		for (i = 0; i < cbNeeded / sizeof(HMODULE); i++)
		{
			...
			if ((*m_GetModuleFileNameEx)(hProcess, hModules[i], moduleInfo.FullPath, _MAX_PATH))
				m_ModuleInfos.push_back(moduleInfo);// AddTail(moduleInfo);
		}
	...
	BOOL SystemModuleInformation::Refresh()
	{
		...
		else
		{
			...
			for (const auto& kv : pi.m_ProcessInfos)
			{
				//pi.m_ProcessInfos.GetNextAssoc( pos, pID, p ); 
				//GetModuleListForProcess( pID );
				GetModuleListForProcess(kv.first);
			}
		}
	...
	BOOL CALLBACK SystemWindowInformation::EnumerateWindows(HWND hwnd, LPARAM lParam)
	{
		...
		if (_this->m_processId == -1 || _this->m_processId == wi.ProcessId)
		{
			...
			if (GetLastError() == 0)
				_this->m_WindowInfos.push_back(wi);// AddTail(wi);
		}
